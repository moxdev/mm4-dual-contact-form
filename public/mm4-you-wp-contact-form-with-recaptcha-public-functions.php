...
function pbc_registration_form() {
	...
	<form name="mm4-contact-form" class="pbc-registration-form" id="mm4-contact-form" method="POST" action="<?php echo get_permalink($options['pbc_registration_form_thank_you_page_id']); ?>" novalidate>
		<label for="first-name">
			<input type="text" name="first-name" id="first-name" class="required" data-error-label="Name" placeholder="NAME">
		</label>
		<label for="email-address">
...
function pbc_registration_form_thank_you_page() {
	...
	$ID = $post->ID;
	$options = get_option( 'mm4-you-wp-contact-form-with-recaptcha-options', array() );
	$ty_page = $options['pbc_registration_form_thank_you_page_id'];
	if( $ty_page && $ID == $ty_page ) {
		require __DIR__ . '/../admin/mm4-you-wp-contact-form-with-recaptcha-contact.php';
	}
}
